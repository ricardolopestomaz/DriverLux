...
<?php require_once __DIR__ . '/../components/header.php'; ?>
  ...
  <div class="opcionais-layout">
    ...
    <main>
      ...
      <section class="bloco-opcional">
        ...
        <?php foreach ($protecoes as $index => $protecao): ?>
          <?php if ($protecao['ativo']): ?>
            <div class="opcao-card opcao-protecao <?= $index === 0 ? 'ativa' : '' ?>" data-id="<?= $protecao['id'] ?>"
              data-nome="<?= htmlspecialchars($protecao['nome']) ?>" data-valor="<?= $protecao['valor_diario'] ?>">
              <div>
      ...
      <section class="bloco-opcional">
        ...
        <?php foreach ($opcoesKm as $index => $km): ?>
          <?php if ($km['ativo']): ?>
            <div class="opcao-card opcao-km <?= $index === 0 ? 'ativa' : '' ?>" data-id="<?= $km['id'] ?>"
              data-nome="<?= htmlspecialchars($km['nome']) ?>" data-valor="<?= $km['valor_diario'] ?>">
              <div>
    ...
    <aside class="resumo-reserva">
      ...
      <div class="linha-resumo" id="resumo-veiculo-imagem-wrap"
        style="text-align: center; border-bottom: 1px solid #333; padding-bottom: 10px; margin-bottom: 10px; display: none;">
        <img id="resumo-veiculo-img" src="" alt="Foto do Veículo"
      ...
      <button class="btn-voltar" id="btn-voltar-veiculos"
